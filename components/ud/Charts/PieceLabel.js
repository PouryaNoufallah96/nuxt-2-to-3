...
(function() {
    ...
    for (var i = 0; i < meta.data.length; i++) {
      ...
      if (this.position === "outside" || this.position === "border") {
        ...
        if (this.position === "border") {
          ...
          rangeFromCentre =
            view.outerRadius - innerRadius + innerRadius + offset;
        }
        position = {
  ...
  PieceLabel.prototype.shouldRenderData = function(view) {
    ...
    return this.chartInstance.config.type === "polarArea"
      ? view.outerRadius !== 0
      : view.circumference !== 0 || this.showZero;
  };
  ...
  PieceLabel.prototype.fillText = function(text, position, fontColor) {
    ...
    if (typeof text === "object") {
      ...
      for (var i = 0; i < lines.length; i++) {
        var y =
          position.y - (this.fontSize / 2) * lines.length + this.fontSize * i;
  ...
  function init(chartInstance) {
    if (chartInstance.options.pieceLabel) {
      ...
      if (
        ...
        count !== chartInstance.pieceLabel.length
      ) {
        chartInstance.pieceLabel = [];
